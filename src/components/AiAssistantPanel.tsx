...
* **Triage Failed Logs**: Provide me with stacktraces, and I will isolate the root cause and propose automated fixes.`,
  ...
  function simulateChromeAiNano(prompt: string, context: any) {
    ...
    const sortedRunsByPassRate = [...runsList].sort((a, b) => (a.passRate ?? 0) - (b.passRate ?? 0));
    const worstRun = sortedRunsByPassRate[0];
    const bestRun = [...runsList].sort((a, b) => (b.passRate ?? 0) - (a.passRate ?? 0))[0];

    // Calculate averages
    const avgPassRate = runsList.length
      ? Math.round(runsList.reduce((sum: number, r: any) => sum + ((r?.passRate ?? 0) ?? 0), 0) / runsList.length)
      : 92;
...
* **Pass Rate:** \`${(worstRun?.passRate ?? 0)}%\` (Failed \`${worstRun.failedCount || 0}\` out of \`${worstRun.testsCount || 10}\` tests)
...
* **Pass Rate:** \`${(bestRun?.passRate ?? 0)}%\` (Passed \`${bestRun.passedCount || 10}\` out of \`${bestRun.testsCount || 10}\` tests)
...
This run executed with maximum container pool optimization and zero reported network/thread latency anomalies.`,
        ...
        chartType: "bar",
        chartData: [...runsList].sort((a, b) => (b.passRate ?? 0) - (a.passRate ?? 0)).slice(0, 5).map(r => ({
          name: r.id,
          value: (r?.passRate ?? 0)
        })),
        tableTitle: "Best Performing Runs",
        tableHeaders: ["Run ID", "Environment", "Pass Rate", "Triggered By", "Duration"],
        tableRows: [...runsList].sort((a, b) => (b.passRate ?? 0) - (a.passRate ?? 0)).slice(0, 3).map(r => [
          r.id,
          r.environment,
          `${(r?.passRate ?? 0)}%`,
          r.triggeredBy || 'Manual',
    ...
    if (p.includes('github') || p.includes('action') || p.includes('workflow') || p.includes('sched') || p.includes('schedule')) {
      ...
      if (scheduledRuns.length > 0) {
        runsSummaryMarkdown = `Currently, AWARE tracks **${scheduledRuns.length}** scheduled run iterations executed in GitHub Actions. Let's inspect the active cross-environment run matrix:\n\n` + 
          scheduledRuns.map((r: any) => `* **${r.id}** (${r.name}): Passed **${r.passedCount || 10}** of **${r.testsCount || 10}** tests in **${r.environment}** with a **${(r?.passRate ?? 0)}%** pass rate (Duration: ${r.duration}).`).join('\n');
      } else {
...
3. **Prod Flakiness Isolation**: Random networking timeouts during scheduled prod executions require robust retry pipelines to avoid intermittent false-negatives.`,
    ...
    if (p.includes('environment') || p.includes('qa') || p.includes('uat') || p.includes('prod') || p.includes('compare')) {
      ...
      const avgQA = qaRuns.length ? Math.round(qaRuns.reduce((sum: number, r: any) => sum + (r?.passRate ?? 0), 0) / qaRuns.length) : 85;
      const avgUAT = uatRuns.length ? Math.round(uatRuns.reduce((sum: number, r: any) => sum + (r?.passRate ?? 0), 0) / uatRuns.length) : 100;
      const avgPROD = prodRuns.length ? Math.round(prodRuns.reduce((sum: number, r: any) => sum + (r?.passRate ?? 0), 0) / prodRuns.length) : 94;
...
What specific diagnostic task would you like me to run?`,
    ...
    if (p.includes('status') || p.includes('summary') || p.includes('overview') || p.includes('report') || p.includes('pipeline') || p.includes('how are we') || p.includes('how is my')) {
      ...
      const runSummaryLines = last3Runs.map((r: any) => `* **${r.id}** (${r.name}): **${(r?.passRate ?? 0)}%** pass rate in **${r.environment}** (Duration: ${r.duration}).`).join('\n');
      return {
...
Based on the current dataset, environment stability is **${avgPassRate > 90 ? "Excellent" : "Nominal, with minor regressions"}**. To optimize pipeline throughput, ensure that retries are enabled on all test folders with a pass rate below 90%.`,
        ...
        chartType: "line",
        chartData: runsList.slice(0, 5).reverse().map((r: any) => ({ name: r.id, value: (r?.passRate ?? 0) })),
...
${slowRuns.slice(0, 3).map((r: any) => `* **${r.id}** (${r.environment}): Duration **${r.duration}** with a pass rate of **${(r?.passRate ?? 0)}%**.`).join('\n') || "*No runs recorded.*"}
...
You can explore specific runs by typing a direct Run ID (e.g., \`RUN-4592-SIM\`), or compare environments by typing *"compare environments"*.`,
          ...
          tableRows: [
            ["QA Staging", String(runsList.filter((r: any) => r.environment === 'QA').length), `${Math.round(runsList.filter((r: any) => r.environment === 'QA').reduce((sum: number, r: any) => sum + (r?.passRate ?? 0), 0) / (runsList.filter((r: any) => r.environment === 'QA').length || 1))}%`, "Active dev cycles"],
            ["UAT Pre-Prod", String(runsList.filter((r: any) => r.environment === 'UAT').length), `${Math.round(runsList.filter((r: any) => r.environment === 'UAT').reduce((sum: number, r: any) => sum + (r?.passRate ?? 0), 0) / (runsList.filter((r: any) => r.environment === 'UAT').length || 1))}%`, "Stable baseline"],
            ["Prod Live", String(runsList.filter((r: any) => r.environment === 'Prod' || r.environment === 'PROD').length), `${Math.round(runsList.filter((r: any) => r.environment === 'Prod' || r.environment === 'PROD').reduce((sum: number, r: any) => sum + (r?.passRate ?? 0), 0) / (runsList.filter((r: any) => r.environment === 'Prod' || r.environment === 'PROD').length || 1))}%`, "Monitored release"]
          ]
        };
      }
...
* **Pass Rate:** \`${(matchingRun?.passRate ?? 0)}%\`
...
#### 💡 Resolution Roadmap:
This run executed under **${matchingRun.environment}** configurations. ${(matchingRun?.passRate ?? 0) < 100 ? "We recommend cross-referencing this with the UAT stable baseline and re-running the failed specs." : "This is a perfect execution with no action required."}`,
        chartTitle: `Run ${matchingRun.id} Execution Coordinates`,
        chartType: "bar",
        chartData: [
          { name: "Pass Rate", value: (matchingRun?.passRate ?? 0) },
          { name: "Fail Rate", value: 100 - (matchingRun?.passRate ?? 0) },
        ...
        tableRows: [
          ...
          ["Pass Percentage", `${(matchingRun?.passRate ?? 0)}%`, (matchingRun?.passRate ?? 0) >= 90 ? "✅ Healthy" : "⚠️ Low"],
...
3. **Automated Retries**: Turn on retry triggers for this folder context to prevent false alerts.`,
    ...
    if (matchedRuns.length > 0) {
      matchText += `\n\n#### 📍 Matching Telemetry Runs Detected (${matchedRuns.length}):\n` + 
        matchedRuns.map((r: any) => `* **Run ID:** \`${r.id}\` | **Env:** \`${r.environment}\` | **Pass Rate:** \`${(r?.passRate ?? 0)}%\` | **Status:** \`${r.status}\``).join('\n');
    }
